...
# 检查本地是否存在 config.yaml
if [ -f "$Conf_Dir/config.yaml" ]; then
    echo "本地已存在 config.yaml，跳过下载。"
    # 将现有的 config.yaml 复制到临时目录
    \cp -a $Conf_Dir/config.yaml $Temp_Dir/clash_config.yaml
else
    # 检查url是否有效
    echo -e '\n正在检测订阅地址...'
    Text1="Clash订阅地址可访问！"
...
# 加载 FORWARD_API_OPENAI_URL 变量，并检查是否为空
Forward_URL=${FORWARD_API_OPENAI_URL:?Error: FORWARD_API_OPENAI_URL variable is not set or empty}

# 从指定的 URL 中提取域名
Forward_Domain=$(echo $Forward_URL | awk -F/ '{print $3}')

# 获取指定域名的 IP 地址
Forward_IP=$(ping -c 1 $Forward_Domain | grep 'PING' | awk -F'(' '{print $2}' | awk -F')' '{print $1}')

# 检查是否成功获取 IP 地址
if [[ -z "$Forward_IP" ]]; then
    echo "Failed to obtain IP address from the specified URL"
    exit 1
fi

# 在 config.yaml 中添加 host 映射
echo -e "\n# Custom host mapping" >> $Conf_Dir/config.yaml
echo "hosts:" >> $Conf_Dir/config.yaml
echo "  'api.openai.com': '$Forward_IP'" >> $Conf_Dir/config.yaml
